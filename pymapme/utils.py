...
def map_fields_from_model(
    ...
    for name, field in destination_model_type.model_fields.items():
        if field.json_schema_extra is None:
        ...
        elif source_path := field.json_schema_extra.get("source"):
            value = map_from_model_field(
                source_model=source_model,
                source_path=source_path,
                sep=field.json_schema_extra.get("source_sep", "."),
                default=_default,
            )
        elif source_func := field.json_schema_extra.get("source_func"):
            ...
            value = source_func(
                source_model, default=field.default or _default, **source_func_context
            )
        else:
            value = getattr(source_model, name, _default)
...
def map_from_model_field(
    ...
    value = reduce(
        lambda val, key: getattr(val, key, default)
